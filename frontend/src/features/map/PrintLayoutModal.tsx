...
export default function PrintLayoutModal({
  ...
  async function handlePrint() {
    ...
    try {
      ...
      if (showAttrib) {
        doc.text('© RakshaGIS — DGDE Survey Platform ', margin, fy + 4 * tScale)
      } else {
  ...
  async function handleExportServer() {
    ...
    try {
      ...
      if (valid.length > 0) {
        ...
        valid.forEach((c) => { try { ctx.drawImage(c, 0, 0) } catch { /* cross-origin */ } })
        mapImageB64 = composite.toDataURL('image/png').split(',')[1]
      }
      ...
      link.href  = url
      link.download = `${(vals.title || projectName).replace(/[^a-zA-Z0-9_-]/g, '_')}_${vals.pageSize}_arcgis.pdf`
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
      window.URL.revokeObjectURL(url)
      message.success('PDF exported — ArcGIS-style layout')
      onClose()
    } catch (err: any) {

...
export default function PrintLayoutModal({
  ...
  async function handlePrint() {
    ...
    try {
      ...
      const legendW  = showLegend && legend.length > 0 ? Math.round(52 * tScale) : 0
      ...
      if (valid.length > 0) {
        ...
        composite.width  = Math.round(valid[0].width  * dpiScale)
        composite.height = Math.round(valid[0].height * dpiScale)
        mapCanvasW = composite.width
        mapCanvasH = composite.height

        const ctx = composite.getContext('2d')!
        ctx.scale(dpiScale, dpiScale)
        valid.forEach((c) => {
          try { ctx.drawImage(c, 0, 0) } catch { /* cross-origin */ }
        })
        // Use PNG — lossless, no compression artefacts on line work
        imgData = composite.toDataURL('image/png')
      }
      ...
      // ── Map image ─────────────────────────────────────────────────────
      if (imgData) {
        doc.addImage(imgData, 'PNG', margin, mapTop, mapW, mapH)
      } else {
      ...
      if (showAttrib) {
        doc.text('© RakshaGIS — DGDE Survey Platform  |  Map data: Survey of India', margin, fy + 4 * tScale)
      } else {
        doc.text('Generated by RakshaGIS — DGDE Survey Platform', margin, fy + 4 * tScale)
      }
      ...
      doc.save(filename)
      message.success('PDF downloaded — high quality output')
      onClose()
    } catch (err: any) {
      console.error(err)
    ...
    <DraggableModal
      ...
      footer={
        <Space>
          ...
          <Button type="primary" icon={<DownloadOutlined />} loading={generating} onClick={handlePrint}>
            Generate PDF
          </Button>
        </Space>
      }
        ...
        {legend.length > 0 && (
          <div style={{ background: '#0a0e1a', border: '1px solid #1a3a5a', borderRadius: 4, padding: '8px 12px' }}>
            <div style={{ color: '#666', fontSize: 11, marginBottom: 6 }}>
              Legend preview — {legend.length} layer{legend.length !== 1 ? 's' : ''}
              {legend.length > 8 && <Tag style={{ marginLeft: 6, fontSize: 10 }}>+{legend.length - 8} more</Tag>}
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px 16px' }}>
              {legend.slice(0, 8).map((item) => (
                <div key={item.name} style={{ display: 'flex', alignItems: 'center', gap: 5, minWidth: 120 }}>
                  <span style={{
                    display: 'inline-block', width: 10, height: 10,
                    background: item.type === 'raster' ? 'linear-gradient(135deg, #a0c4e0 50%, #5588aa 50%)' : item.color,
                    borderRadius: 2, flexShrink: 0, border: '1px solid rgba(255,255,255,0.15)',
                  }} />
                  <span style={{ color: '#bbb', fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 100 }}>
                    {item.name}
                  </span>
                </div>
              ))}

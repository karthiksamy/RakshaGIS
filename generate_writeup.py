...
add_body(doc,
    ...
    'document editing, AI-assisted document processing, 3D terrain analysis, offline-first field '
    'companion, C2PA provenance watermarking, and map publishing into a single role-gated '
...
for label, detail in [
    ...
    ('Strict office-level data isolation',
     'Enforce row-level isolation so each office sees only its own content. '
     'DGDE and PDDE offices reach subordinate offices exclusively through the published Map Viewer — '
     'they cannot view, add, edit, or delete sub-office projects, documents, or features. '
     'A single central hq_level() helper enforces this across all API endpoints.'),
    ('Provenance and authenticity',
     'All exported files (terrain PNGs, GeoJSON, documents, report PDFs) carry C2PA cryptographic '
     'provenance assertions and Living Provenance DNA steganographic watermarks binding the export '
     'to the project, uploader, and timestamp.'),
...
for text in [
    ...
    'SUPERADMIN accounts attached to a DGDE or PDDE organisation are subject to the same isolation rules as any other HQ user — only a global superadmin (no organisation assigned) retains unrestricted system-level access.',
]:
    add_plain_bullet(doc, text)
...
# 3.13
add_heading(doc, '3.13  Mandatory Two-Factor Authentication (TOTP)', level=2)
add_body(doc,
    'All accounts are required to enrol in and use TOTP-based two-factor authentication. '
    'There is no bypass — a session token is never issued without a valid OTP.')
for text in [
    'First login: the backend returns a requires_2fa_setup status; the login page presents a QR code '
    '(base32 URI) generated by pyotp. The user scans it with any TOTP app (Google Authenticator, Authy, '
    'Microsoft Authenticator) and confirms the first code. Only then is the session token issued.',
    'Subsequent logins: the backend returns requires_2fa; the login page prompts for a 6-digit OTP. '
    'Django validates it against the stored secret with a ±60 s tolerance (valid_window=2) to handle minor clock drift.',
    'Backend model: TOTPDevice stores per-user secret; two dedicated endpoints — '
    '/api/accounts/auth/2fa/setup-begin/ and /api/accounts/auth/2fa/setup-complete/ — handle the enrolment flow.',
    'The login endpoint never issues tokens directly; it always returns a pending-2fa status that the '
    'frontend dispatches to the appropriate step.',
]:
    add_plain_bullet(doc, text)

# 3.14
add_heading(doc, '3.14  3D Terrain Analysis & DEM Tools', level=2)
add_body(doc,
    'The Cesium.js 3D terrain viewer at /terrain has been extended with a comprehensive suite of '
    'Digital Elevation Model (DEM) analysis tools and vector overlay capabilities.')
for text in [
    '16 DEM analysis tools accessible from the left-side toolbar: Slope, Aspect, Hillshade, '
    'Curvature, TPI (Topographic Position Index), TRI (Terrain Ruggedness Index), '
    'Roughness, Flow Direction, Flow Accumulation, Viewshed, Least-Cost Path, '
    'Cut/Fill, Solar Radiation, Flood Simulation, Landslide Risk, and Erosion Potential.',
    'Each analysis tool produces a colour-rendered scene PNG with a legend panel (colour ramp + breakpoints) '
    'and a statistics panel (min/avg/max/std) composed below the 3D scene — no overlap with the terrain view.',
    'Slope analysis over a user-drawn rectangle: samples a configurable grid (up to 50×50 = 2,500 points) '
    'via the Elevation Lookup API; renders a colour-coded slope overlay; shows min/avg/max in degrees.',
    'Vector file upload into the 3D viewer: upload Shapefile (.zip), GeoJSON, KML, KMZ, or GeoPackage; '
    'Django backend parses with Fiona and returns WGS84 GeoJSON; Cesium renders as a 3D GeoJsonDataSource overlay.',
    'Slope analysis from vector: after loading a vector layer, click Analyze to run slope analysis '
    'over the layer bounding box in one click.',
    'All exports (PNG, GeoJSON, CSV) are watermarked before download via C2PA + LP-DNA pipeline.',
    'Offline SRTM setup: ./setup_terrain.sh downloads and converts tiles; '
    'scripts/generate_terrain_layer.py scans the tile tree and writes the correct layer.json availability index '
    'required by the Cesium quantized-mesh terrain client.',
    'PWA and Coordinate Tool panels are collapsed to icon-only by default; clicking the icon expands the panel.',
]:
    add_plain_bullet(doc, text)

# 3.15
add_heading(doc, '3.15  C2PA & Living Provenance DNA Watermarking', level=2)
add_body(doc,
    'All exports from RakshaGIS carry cryptographic and steganographic provenance watermarks '
    'that establish authenticity and bind the export to its source project and creator.')
for text in [
    'C2PA (Content Authenticity Initiative): embeds cryptographic provenance assertions in the file '
    'metadata that can be verified by Adobe Content Credentials and other C2PA-aware tools. '
    'Applied to image and office-format exports.',
    'LP-DNA (Living Provenance DNA): invisible steganographic metadata embedded in the file binary '
    'that binds the export to the project ID, project number, uploader username, and timestamp. '
    'Applied to all export types including PDF reports.',
    'Central watermark pipeline at /api/core/watermark-file/ — all terrain analysis PNGs, '
    'GeoJSON downloads, CSV exports, uploaded documents, and AI-generated report PDFs pass through this endpoint.',
    'Frontend utility watermarkAndDownload() (frontend/src/utils/watermarkDownload.ts) posts the '
    'file to the watermark endpoint and triggers a browser download of the watermarked result.',
]:
    add_plain_bullet(doc, text)

# 3.16
add_heading(doc, '3.16  Offline PWA Field Companion', level=2)
add_body(doc,
    'The Field Companion (/field-companion) is a Progressive Web App designed for surveyors '
    'operating in areas with intermittent or no network connectivity.')
for text in [
    'Offline-first OpenLayers map: project layers are cached to IndexedDB when online; the map continues to function without a network.',
    'Project caching: tap "Cache for offline" on any project to store its features, survey areas, and attribute templates in the browser\'s IndexedDB.',
    'GPS tracking: real-time position indicator on the map; tap any map point to add a geo-referenced note with automatically captured coordinates.',
    'Offline feature queue: features added while offline are stored in a local outbox (IndexedDB) and synced automatically when connectivity is restored.',
    'Pending-sync badge: the PWA panel icon on the Map toolbar shows the count of features queued for sync.',
    ...
    'IndexedDB helpers are in frontend/src/utils/offlineStore.ts: cacheProject, getCachedProject, listCachedProjects, queueFeature, getOutbox, removeFromOutbox.',
]:
    add_plain_bullet(doc, text)

# 3.17
add_heading(doc, '3.17  Per-Feature Comment Threads', level=2)
add_body(doc,
    'Any GIS feature carries a threaded discussion that allows surveyors, checkers, and approvers '
    'to collaborate on specific spatial elements without leaving the map.')
for text in [
    'Comments stored in the FeatureComment model (feature FK, user, text, created_at).',
    'Displayed in the Feature Info drawer on the Map page with role-coloured user tags and an inline reply box.',
    'REST API: GET/POST /api/projects/feature-comments/?feature=<id>.',
    'Visible to all users with access to the project (respects org-level isolation).',
]:
    add_plain_bullet(doc, text)

# 3.18
add_heading(doc, '3.18  Attribute Auto-Validator', level=2)
add_body(doc,
    'Every shapefile import automatically triggers a validation pass after import completes, '
    'ensuring data quality before the project advances through the workflow.')
for text in [
    'Checks performed: duplicate feature IDs, missing required attributes (per project AttributeTemplate), '
    'zero-area polygons, and features whose bounding box lies entirely outside India.',
    'Results stored in ShapefileImport.validation_warnings (JSONField — list of warning dicts).',
    'Summary shown in the Import Status UI; an AI review task is also queued to check attribute '
    'completeness and suggest corrections via the local LLM.',
    'Validation runs in the same Celery task as the import — no additional user action required.',
]:
    add_plain_bullet(doc, text)

# 3.19
add_heading(doc, '3.19  AI Survey Report Generator', level=2)
add_body(doc,
    'Generate a full AI-authored survey report for any project from the Reports page or AI Chat.')
for text in [
    'The backend gathers project statistics (area counts by status, feature counts, document inventory, '
    'workflow history) and asks the local LLM to write a structured narrative with findings and recommendations.',
    'Output: formatted PDF (LP-DNA watermarked) stored as a project document + .docx '
    'automatically opened in OnlyOffice for review and annotation.',
    'Report type AI_SUMMARY added to ReportSchedule; accessible from Reports page or triggered via the AI Assistant.',
    'Fully local — inference runs on-premise via Ollama with no internet dependency.',
]:
    add_plain_bullet(doc, text)

# 3.20
add_heading(doc, '3.20  Office Drilldown Dashboard', level=2)
add_body(doc,
    'DGDE and PDDE users have access to a hierarchical statistics dashboard showing aggregated '
    'activity across the office tree without exposing per-project content.')
for text in [
    'DGDE drilldown: national totals → click any PDDE command → click any DEO within that command.',
    'PDDE drilldown: own command totals → click any DEO within the command.',
    'Each level shows: project counts by status, survey area counts, feature counts, document counts, active user counts.',
    'Breadcrumb navigation; DEO is the terminal level — CEO/ADEO sub-office details are not listed.',
    'Aggregates are computed in OrgDrilldownView (/api/dashboard/org-drilldown/) using org.get_subtree_ids() scoped to the user\'s isolation level.',
    'No project names, document titles, or feature data are ever exposed — only aggregate counts.',
]:
    add_plain_bullet(doc, text)
...
tech_rows = [
    ...
    ('Watermarking',          'C2PA SDK + custom LP-DNA steganography', 'Provenance watermarking on all terrain and document exports'),
    ('TOTP 2FA',              'pyotp',                                  'Mandatory TOTP two-factor authentication for all accounts'),
    ('Offline PWA',           'IndexedDB (browser-native)',             'Offline feature queue, project cache, GPS sync for Field Companion'),
    ('3D globe',              'Cesium.js 1.141',                        '3D terrain viewer with 16 DEM analysis tools and vector overlay'),
...
for label, detail in [
    ...
    ('Mandatory TOTP 2FA',
     'All user accounts are required to enrol in and pass TOTP two-factor authentication. '
     'First login triggers QR-code enrolment; every subsequent login requires a 6-digit OTP. '
     'No session token is ever issued without a valid OTP.'),
    ('C2PA + LP-DNA watermarking',
     'All terrain analysis exports (PNG, GeoJSON, CSV), uploaded documents, and AI-generated '
     'report PDFs carry C2PA cryptographic provenance assertions and Living Provenance DNA '
     'steganographic watermarks that bind the file to its project and creator.'),
    ('AI Survey Report Generator',
     'One-click AI-authored survey report: the local LLM writes a structured narrative from '
     'project statistics, feature counts, and document inventory. Output is a watermarked PDF '
     'plus an editable .docx opened directly in OnlyOffice.'),
    ('Attribute Auto-Validator',
     'Automatic post-import validation checks duplicate IDs, missing required attributes, '
     'zero-area polygons, and out-of-India features. Results are stored in '
     'ShapefileImport.validation_warnings and shown in the Import Status UI.'),
    ('Offline PWA Field Companion',
     'A Progressive Web App at /field-companion provides GPS-guided feature capture in the field '
     'with IndexedDB caching, offline feature queue, and automatic sync on reconnection. '
     'A pending-sync badge on the Map toolbar shows the outbox count.'),
    ('Per-Feature Comment Threads',
     'Any GIS feature carries a threaded discussion (FeatureComment model) shown in the '
     'Feature Info drawer with role-coloured tags and an inline reply box.'),
    ('Office Drilldown Dashboard',
     'DGDE and PDDE users have a hierarchical aggregates dashboard: national → command → DEO, '
     'navigable with breadcrumb. Aggregate counts only — no per-project content is exposed.'),
    ('Strict HQ data isolation',
     'DGDE, PDDE, and org-attached superadmins see only their own office content. '
     'Subordinate office projects, documents, features, and folders are never visible to HQ users; '
     'published maps reach them only through the Map Viewer. Enforced centrally by hq_level() '
     'and org_queryset_filter() across all API viewsets, dashboard, search, AI, reports, and workflow.'),
    ('3D vector file overlay and analysis',
     'Shapefile, GeoJSON, KML, KMZ, and GeoPackage files can be uploaded directly into the '
     '3D Terrain viewer; Cesium renders them as a 3D overlay and can trigger slope analysis '
     'over the vector bounding box in one click.'),
]:
...
for label, detail in [
    ...
    ('React Native mobile client',
     'Native iOS/Android app with biometric auth, higher-fidelity GPS integration, '
     'and background sync — extending the existing PWA offline model to native performance.'),
